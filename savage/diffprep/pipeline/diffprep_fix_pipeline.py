...
from diffprep.TFs.mv_imputer import NumMVIdentity, CatMVIdentity
...
class Transformer(nn.Module):
    ...
    def init_parameters(self):
        ...
        if self.init_tf_option is None:
            ...
            init_tf_probs = torch.ones(self.out_features, self.num_tf_options) * ((1-self.init_p) / (self.num_tf_options - 1))
    ...
    def forward(self, X, is_fit, X_type, max_only=False, require_grad=True):
        ...
        X_output = self.select_X_sample(X, X_trans, max_only, require_grad)
        return X_output
...
class DiffPrepFixPipeline(nn.Module):
    ...
    def forward(self, X, is_fit, X_type, resample=False, max_only=False, require_grad=True):
        ...
        for transformer in self.pipeline:
            ...
            # X_output_numpy = X_output.detach().cpu().numpy() if X_output.requires_grad else X_output.cpu().numpy()
        return X_output
