...
def objective_fuc(trial, col_ids, budget, baseline_auc, pattern_col_list, error_type, dataset, model_name, objective, sens_attr, cleaning):
    lb_list = []
    ub_list = []
    X_train, X_test, y_train, y_test = load(dataset)
    ...
    for col in current_pattern_col_list:
        if col == 'Y':
            ...
            if pd.api.types.is_integer_dtype(X_train_orig[col]):
                ...
                mv_lb = trial.suggest_float(col + '_mv_lb', X_train_orig[col].min(), X_train_orig[col].max())
                mv_interval = trial.suggest_float(col + '_mv_int', 0, X_train_orig[col].max() - mv_lb)
            lb_list.append(mv_lb)
            mv_ub = mv_interval + mv_lb
            ub_list.append(mv_ub)
    ...
    mv_num = min(mv_pattern_len, budget // 4)

    mv_errs = []
    for col_id in col_ids:
        mv_errs.append(MissingValueError(col_id, mv_pattern, mv_num / mv_pattern_len))

    injecter = Injector(error_seq=mv_errs)
...
def run_one_setting(setting_dict):
    ...
    missing_cols = setting_dict['missing_cols']
    budget = setting_dict['budget']
    error_type = setting_dict['error_type']
    n_trials = setting_dict['n_trials']
    cleaning = setting_dict['cleaning']

    col_ids = [list(X_train_orig.columns).index(missing_col) for missing_col in missing_cols]
    print(f"Evaluating seed: {seed} for {missing_cols} with {budget} NaNs...")
    study = optuna.create_study(direction='minimize', sampler=TPESampler(seed=seed))
    study.optimize(
        lambda trial: objective_fuc(trial, col_ids, budget, baseline_auc, pattern_col_list, error_type, dataset,
                                    model_name, objective, sens_attr, cleaning),
        n_trials=n_trials)
    ...
    return seed, tuple(missing_cols), budget, auc, spd, eo, acc, f1, col_list, lb_list, ub_list
...
def merge_results(final_results, result_filename):
    ...
    final_results_str_keys = {str(k): v for k, v in final_results.items()}

    for col in final_results_str_keys:
        move_zero_to_front(final_results_str_keys[col])
        for metric in ['AUC', 'SPD', 'EO', 'ACC', 'F1']:
            existing_results[col][metric]['budgets'].extend(final_results_str_keys[col][metric]['budgets'])
            existing_results[col][metric]['means'].extend(final_results_str_keys[col][metric]['means'])
            existing_results[col][metric]['stds'].extend(final_results_str_keys[col][metric]['stds'])

        for key in ['col_list', 'lb_list', 'ub_list']:
            existing_results[col][key].extend(final_results_str_keys[col][key])
...
def main():
    ...
    X_train_orig, X_test_orig = X_train.copy(), X_test.copy()
    X_test_orig.reset_index(drop=True, inplace=True)
    nan_counts = [0] if not override else [0] + [int(pct * len(X_train_orig)) for pct in np.arange(error_pct, error_pct * 50 + error_pct, error_pct)]
    ...
    settings_list = [
        {'dataset': dataset, 'model_name': model_name, 'error_type': error_type, 'baseline_auc': baseline_auc,
         'seed': seed, 'missing_cols': error_cols, 'budget': budget, 'n_trials': n_trials,
         'pattern_col_list': pattern_cols,
         'objective': attack_objective, 'sens_attr': sens_attr, 'cleaning': cleaning}
        for seed in seed_values for budget in nan_counts]

    final_results = {
        str(tuple(error_cols)): {
    ...
    for result in results:
        ...
        col_str = str(tuple(col))

        for metric_name, value in metrics.items():
            if budget not in final_results[col_str][metric_name]['budgets']:
                final_results[col_str][metric_name]['budgets'].append(budget)
                final_results[col_str][metric_name]['means'].append([])
                final_results[col_str][metric_name]['stds'].append([])

            index = final_results[col_str][metric_name]['budgets'].index(budget)
            final_results[col_str][metric_name]['means'][index].append(value)

        final_results[col_str]['col_list'][budget].append(col_list)
        final_results[col_str]['lb_list'][budget].append(lb_list)
        final_results[col_str]['ub_list'][budget].append(ub_list)
    ...
    if error_type in ['Label', 'Sampling']:
        final_results = {'full': final_results[str(tuple(error_cols))]}
