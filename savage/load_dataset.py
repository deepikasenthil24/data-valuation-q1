...
def load_adult(sample=False):
    ...
    if sample:
        df_train = df_train.sample(frac=0.05, random_state=1)
...
def load_employee(sample=False):
...
def load_credit_card(sample=False):
    df = pd.read_csv('credit_card_default.csv')
    if sample:
        new_df = df.sample(frac=0.1, random_state=1)
        new_df.reset_index(drop=True)
    else:
        new_df = df
        
    X_train, X_test, y_train, y_test = process_credit_card(new_df)
    return X_train, X_test, y_train, y_test
...
def load_med_insurance():
    df = pd.read_csv('insurance/insurance.csv')
    X = df.drop(['charges', 'sex', 'smoker', 'region'], axis=1)
    y = df['charges']
    y_cat = pd.Series(np.where(y > 10000, 1, 0))
    y_cat.name = "charges"
...
def load_concrete():
    ...
    y = y["Concrete compressive strength"]
    y_cat = pd.Series(np.where(y > 30, 1, 0))
    y_cat.name = "Concrete compressive strength"
...
def load(dataset, preprocess=True, row_num=10000, attr_num=30, sample=False):
    if dataset == 'compas':
        ...
        return load_employee(sample=sample)
    elif dataset == 'credit_card':
        return load_credit_card(sample=sample)
    elif dataset == 'churn':
        return load_churn()
    elif dataset == 'hr':
        return load_hr()
    elif dataset == 'car_insurance':
        return load_insurance()
    elif dataset == 'insurance':
        return load_med_insurance()
