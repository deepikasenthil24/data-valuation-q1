...
def initialize_model_and_baseline_auc(dataset, model_name, cleaning, sens_attr):
    ...
    if cleaning == 'boostclean':
        ...
        # baseline_auc and label worst AUC
        # pruning rate
        print(baseline_auc)
        ...
        baseline_auc = roc_auc_score(y_test,
                                     DecisionTreeClassifier(random_state=42).fit(X_train_orig, y_train).predict_proba(
                                         X_test_orig)[:, 1])
    else:
    ...
    correlation_matrix.to_csv('feature_correlation_matrix.txt', sep='\t')
...
def main():
    ...
    parser.add_argument('--pattern_cols', type=str, nargs='*', help='Pattern selection columns', default=[])
    ...
    X_train_orig, X_test_orig = X_train.copy(), X_test.copy()
    X_test_orig.reset_index(drop=True, inplace=True)
    nan_counts = [0] if not override else [0] + [int(pct * len(X_train_orig)) for pct in
                                                 np.arange(error_pct, error_pct * 1 + error_pct, error_pct)]
