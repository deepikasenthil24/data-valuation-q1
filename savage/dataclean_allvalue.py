...
def initialize_model_and_baseline_auc(dataset, model_name, cleaning, sens_attr):
    ...
    if cleaning == 'boostclean':
        ...
        baseline_auc = roc_auc_score(
            y_test,
            get_model(model_name, seed=42)
            .fit(X_train_orig, y_train)
            .predict_proba(X_test_orig)[:, 1]
        )
    else:
...
def objective_func(trial, col_id, budget, baseline_auc, pattern_col_list, error_type, 
                   dataset, model_name, objective, sens_attr, cleaning, seed):
    #     t0 = time.time()
    ...
    for col in current_pattern_col_list:
        if col == 'Y':
            ...
            if pd.api.types.is_integer_dtype(X_train_orig[col]):
                ...
                print("Before mapping to actual values: mv_lb_percent: {}, mv_ub_percent: {}".format(mv_lb_percent, mv_ub_percent))
                mv_lb = map_percent_to_value(X_train_orig[col], mv_lb_percent)
                mv_ub = map_percent_to_value(X_train_orig[col], mv_ub_percent)
                print("After mapping to actual values: mv_lb: {}, mv_ub: {}".format(mv_lb, mv_ub))
            lb_list.append(mv_lb)
            ub_list.append(mv_ub)
...
def run_one_setting(setting_dict):
    ...
    print("Evaluating seed: {} for {} with {} NaNs...".format(seed, missing_col, budget))
    study = optuna.create_study(direction='minimize', sampler=TPESampler(seed=seed))
    study.optimize(
        lambda trial: objective_func(trial, col_id, budget, baseline_auc, pattern_col_list, 
                                     error_type, dataset, model_name, objective, 
                                     sens_attr, cleaning, seed),
        n_trials=n_trials)

    best_trial = study.best_trial
    auc = best_trial.user_attrs['auc']
    spd = best_trial.user_attrs['spd']
    eo = best_trial.user_attrs['eo']
    acc = best_trial.user_attrs['accuracy']
    f1 = best_trial.user_attrs['f1_score']
    col_list = best_trial.user_attrs['col_list']
    lb_list = best_trial.user_attrs['lb_list']
    ub_list = best_trial.user_attrs['ub_list']
    ...
    for metric in ["AUC", "SPD", "EO", "ACC", "F1"]:
        ...
        if 0 in data[metric]['budgets']:
            ...
            if idx_0 != 0:
                ...
                data[metric]['medians'].insert(0, data[metric]['medians'].pop(idx_0))
                data[metric]['q1'].insert(0, data[metric]['q1'].pop(idx_0))
                data[metric]['q3'].insert(0, data[metric]['q3'].pop(idx_0))
                data[metric]['values'].insert(0, data[metric]['values'].pop(idx_0))

    for key in ["col_list", "lb_list", "ub_list"]:
        if data[key]:
            if 0 in data[key]:
                idx_0 = list(data[key].keys()).index(0)
                if idx_0 != 0:
                    data[key][0], data[key][idx_0] = data[key][idx_0], data[key][0]

def merge_results(final_results, result_filename):
    with open(result_filename, 'r') as json_file:
        existing_results = json.load(json_file)

    # move_zero_to_front(final_results)
    for metric in ['AUC', 'SPD', 'EO', 'ACC', 'F1']:
        existing_results[metric]['budgets'].extend(final_results[metric]['budgets'])
        existing_results[metric]['means'].extend(final_results[metric]['means'])
        existing_results[metric]['stds'].extend(final_results[metric]['stds'])

    for key in ['col_list', 'lb_list', 'ub_list']:
        existing_results[key].extend(final_results[key])
...
def main():
    ...
    parser.add_argument('--error_pcts', type=float, nargs='+', required=True, help='Error number percentage (default: 10%)')
    ...
    X_train_orig, X_test_orig = X_train.copy(), X_test.copy()
    X_test_orig.reset_index(drop=True, inplace=True)
    nan_counts = [int(pct * len(X_train_orig)) for pct in error_pcts]
    # nan_counts = [0] if not override else [0] + [int(pct * len(X_train_orig)) for pct in error_pcts]
    ...
    # final_results = {
    #     col: {
    #         'AUC': {'means': [], 'stds': [], 'medians': [], 'q1': [], 'q3': [], 'budgets': [], 'values': []},
    #         'SPD': {'means': [], 'stds': [], 'medians': [], 'q1': [], 'q3': [], 'budgets': [], 'values': []},
    #         'EO': {'means': [], 'stds': [], 'medians': [], 'q1': [], 'q3': [], 'budgets': [], 'values': []},
    #         'ACC': {'means': [], 'stds': [], 'medians': [], 'q1': [], 'q3': [], 'budgets': [], 'values': []},
    #         'F1': {'means': [], 'stds': [], 'medians': [], 'q1': [], 'q3': [], 'budgets': [], 'values': []},
    ...
    for result in results:
        ...
        for metric_name, value in metrics.items():
            if budget not in final_results[metric_name]['budgets']:
                final_results[metric_name]['budgets'].append(budget)
                # store the best value for each seed
                final_results[metric_name]['values'].append([])

            index = final_results[metric_name]['budgets'].index(budget)
            final_results[metric_name]['values'][index].append(value)

        final_results['col_list'][budget].append(col_list)
        final_results['lb_list'][budget].append(lb_list)
        final_results['ub_list'][budget].append(ub_list)

        # for metric_name, value in metrics.items():
        #     if budget not in final_results[col][metric_name]['budgets']:
        #         final_results[col][metric_name]['budgets'].append(budget)
        #         # store the best value for each seed
        #         final_results[col][metric_name]['values'].append([])

        #     index = final_results[col][metric_name]['budgets'].index(budget)
        #     final_results[col][metric_name]['values'][index].append(value)
    ...
    # for col in final_results:
    for metric in ['AUC', 'SPD', 'EO', 'ACC', 'F1']:
        budgets = final_results[metric]['budgets']
        final_results[metric]['means'] = []
        final_results[metric]['stds'] = []
        final_results[metric]['medians'] = []
        final_results[metric]['q1'] = []
        final_results[metric]['q3'] = []
        # budgets = final_results[col][metric]['budgets']
        # final_results[col][metric]['means'] = []
        # final_results[col][metric]['stds'] = []
        # final_results[col][metric]['medians'] = []
        # final_results[col][metric]['q1'] = []
        # final_results[col][metric]['q3'] = []

        for i, budget in enumerate(budgets):
            values = final_results[metric]['values'][i]
            final_results[metric]['means'].append(np.mean(values))
            final_results[metric]['stds'].append(np.std(values))
            final_results[metric]['medians'].append(np.median(values))
            final_results[metric]['q1'].append(np.quantile(values, 0.25))
            final_results[metric]['q3'].append(np.quantile(values, 0.75))
            # values = final_results[col][metric]['values'][i]
            # final_results[col][metric]['means'].append(np.mean(values))
            # final_results[col][metric]['stds'].append(np.std(values))
            # final_results[col][metric]['medians'].append(np.median(values))
            # final_results[col][metric]['q1'].append(np.quantile(values, 0.25))
            # final_results[col][metric]['q3'].append(np.quantile(values, 0.75))

    for key in ['col_list', 'lb_list', 'ub_list']:
        # print(key, final_results[key])
        sorted_budgets = sorted(final_results[key].keys())
        final_results[key] = [final_results[key][budget] for budget in sorted_budgets]
    ...
    result_filename = "jsons/{}_results_{}_{}_{}_{}.json".format(dataset, cleaning, error_type, attack_objective,
                                                                 model_name)
