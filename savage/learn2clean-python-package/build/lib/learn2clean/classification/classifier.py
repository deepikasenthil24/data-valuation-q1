...
class Classifier():
    ...
    * k_folds: int, default = 5, number of folds for cross-validation

    * verbose: Boolean,  default = 'False' otherwise display the list of
        duplicate rows that have been removed.
    """

    def __init__(self, dataset, target, strategy='NB', k_folds=5,
                 verbose=False):
...
#             auc = results['mean_test_score'][best_index]
...
#             print("Avg AUC of CART classification for",
...
#         return auc
    ...
    def CART_classification(self, dataset, target):
        ...
        if (len(X_train.columns) <= 1) or (len(X_train) < k):
            ...
            X_val = dataset['val'].select_dtypes(['number']).dropna()

            X_test = dataset['test'].select_dtypes(['number']).dropna()

            if (isinstance(self.dataset['target_val'], dict)):

                y_val = dataset['target'].loc[X_val.index]

            else:

                y_val = dataset['target_val'].loc[X_val.index]
            ...
            if target in X_val.columns.values:
            ...
            gs = GridSearchCV(model, cv=cv, param_grid=params, scoring='accuracy')
...
#                 accuracy = clf.score(X_test, y_test)
            ...
            print("Avg Val AUC of CART classification for", k, "cross-validation :", val_auc)
