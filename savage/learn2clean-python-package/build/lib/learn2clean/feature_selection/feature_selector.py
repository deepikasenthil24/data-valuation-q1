...
#         print(dn[to_keep])
        ...
        if (not isinstance(self.dataset['val'], dict)):
            # if not self.dataset['test'].empty:

            df_val = pd.DataFrame.from_dict(self.dataset['val'])

            to_keep = list(set(to_keep) & set(df_val.columns))

            fsd['val'] = df_val[to_keep]
