...
def logistic_loss_torch(model, x, y_true, sample_weights=None, reg=1):
    if isinstance(y_true, int) or isinstance(y_true, np.ndarray):
        ...
        y_pred = model(x)
        return reg*model.C * torch.norm(convert_grad_to_tensor(list(model.parameters())), p=2)**2+binary_cross_entropy(y_pred, y_true, sample_weights)
...
def svm_loss_torch(model, x, y_true, sample_weights=None):
    ...
    if sample_weights is None:
        return model.C * torch.norm(convert_grad_to_tensor(list(model.lr.parameters())), p=2)**2/2+torch.mean(model.smooth_hinge(1 - y_true*model.decision_function(x)))
    else:
        return model.C * torch.norm(convert_grad_to_tensor(list(model.lr.parameters())), p=2)**2/2+torch.mean(model.smooth_hinge(1 - y_true*model.decision_function(x))*sample_weights)
