...
def get_indices(singleloader):
    ...
    if hasattr(sampler, 'indices'):
        # SubsetRandomSampler or custom sampler
        return sampler.indices
    else:
        # RandomSampler or similar — generate indices on the fly
...
def get_OT_dual_sol(feature_extractor, trainloader, testloader, training_size=10000, p=2, resize=32, device='cpu'):
...
def compute_dual(feature_extractor, trainloader, testloader, training_size, shuffle_ind, p=2, resize=32, device='cpu'):
    # to return 2
    ...
    # to return 1
    # OT Dual calculation
    dual_sol = get_OT_dual_sol(feature_extractor, trainloader, testloader, p=2, resize=32, 
                               device='cpu' #device='cuda'
                               )
...
#                   'detection rate: ', str(found / poisoned), 'baseline = '+str(vari*0.2*0.9))
    ...
    print("Number of poisoned images: ", poison_cnt, " out of ", trsize,".")
...
#### WINE DATA 
class WineFeatureExtractor(nn.Module):
    def __init__(self, input_dim=9, embedding_dim=32):
        super(WineFeatureExtractor, self).__init__()
...
def load_wine_feature_extractor(device, input_dim=9, embedding_dim=32):
    model = WineFeatureExtractor(input_dim, embedding_dim).to(device)
    model.eval()
    return model